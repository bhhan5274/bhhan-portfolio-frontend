...
import { AddProjectRequest, useProject, useUpdateProject } from '@/hooks/project/projectHooks';
import { useSkills } from '@/hooks/skill/skillHooks';
import {
  convertUrlsToFiles,
  findSkillsByExactNames,
  formatDate,
  parseSeoulDate,
...
export default function Page({ params }: { params: Promise<{ title: string }> }) {
  ...
  const { data: project, error: projectError, isPending: projectPending } = useProject(title);
    ...
    if (project) {
      const init = async () => {
        form.setValues({
          ...
          images: project.imageGallery.images,
          skills: project.skillList.images,
        });

        setNumber(`${project.member}`);
        setDate([parseSeoulDate(project.period.startDate), parseSeoulDate(project.period.endDate)]);
        setContent(project.description);
        setSkillList(project.skillList.images.map((item) => item.name));

        const files = await convertUrlsToFiles(project.imageGallery.images);
        setImages(files);
      };
      init();
    }
  ...
  useNotification({
    status: updateProjectError,
    title: '프로젝트 업데이트 실패',
    description: 'API서버를 확인하세요.',
    color: 'red',
  });

  useRefetchCallback(updateProjectData, () => {
    notifications.show({
      title: '프로젝트 업데이트',
      message: '프로젝트를 업데이트 했습니다.',
      color: 'blue',
    });
  ...
  const handleAdd = (addProjectRequest: AddProjectRequest) => {
    updateProject(addProjectRequest);
  };
    ...
    <Box>
      <Stack pos="relative" align="stretch" maw={{ base: '100%', xl: 1100 }} mx="auto" mt="lg">
        <form onSubmit={form.onSubmit(handleAdd, handleAddError)}>
          <LoadingOverlay
            visible={imageDeleteLoading || imageListLoading || projectPending}
            zIndex={1000}
